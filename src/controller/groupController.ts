...
export const createGroup = async (
  ...
  try {
    ...
    if (!name || !ownersId || !category || !description || !imagePath) {
      next(
        new ApiError(
          "name, ownersId, category, description, and imagePath are required",
          400
        )
      );
      return;
...
export const joinGroup = async (
  ...
  try {
    ...
    const joined = await joinGroupById({
      id: Number(id),
...
export const addMemberToGroup = async (
  ...
  try {
    ...
    const accepted = await acceptJoinRequest({
      id: Number(id),
      userId: Number(userId),
    ...
    if (!accepted) {
      next(
        new ApiError(
          "Group not found, you are not the owner, or join request does not exist",
          404
        )
      );
      return;
...
export const removeMemberFromGroup = async (
  ...
  try {
    ...
    if (!removed) {
      next(
        new ApiError(
          "Group not found, you are not the owner, or member does not exist",
          404
        )
      );
      return;
...
export const getContentFromGroup = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const content = await getContentFromModel({
      id: Number(id),
      userId: Number(userId),
    });
    if (content === null) {
...
export const getAllMembers = async (
  ...
  try {
    ...
    const isMember = await checkGroupMember({
      id: Number(id),
      userId: Number(userId),
    });
    if (!isMember) {
