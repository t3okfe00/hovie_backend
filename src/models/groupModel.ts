...
export const joinGroupById = async (
  ...
  const [existingRequest] = await db
    ...
    .where(
      and(
        eq(joinrequests.groupsId, groupData.id),
        eq(joinrequests.usersId, groupData.userId)
      )
    );
  if (existingRequest) {
    return false;
  ...
  await db
    .insert(joinrequests)
    .values({ groupsId: groupData.id, usersId: groupData.userId });
  return true;
};
...
const isGroupMember = async (groupData: UidIdGroupInput): Promise<boolean> => {
  const [member] = await db
    ...
    .where(
      and(
        eq(groupmembers.groupsId, groupData.id),
        eq(groupmembers.usersId, groupData.userId)
      )
    );
  return !!member;
};
...
export const acceptJoinRequest = async (
  ...
  await db
    .insert(groupmembers)
    .values({
      groupsId: groupData.id,
      usersId: groupData.userId,
      role: "member",
    });
  return true;
};

export const declineJoinRequestById = async (
  groupId: number,
  userId: number
): Promise<boolean> => {
  const [request] = await db
    ...
    .where(
      and(
        eq(joinrequests.groupsId, groupId),
        eq(joinrequests.usersId, userId),
        eq(joinrequests.status, "pending")
      )
    );
  if (!request) {
...
export const removeMember = async (
  ...
  if (groupData.ownerId !== null) {
    ...
    if (
      !(await isGroupMember({ id: groupData.id, userId: groupData.userId }))
    ) {
      return false;
    }
  }
...
export const addContent = async (
  groupData: CreateGroupContentInput
): Promise<boolean> => {
  if (
    !(await isGroupMember({
      id: groupData.groupsId,
      userId: groupData.addedByUserId,
    }))
  ) {
    return false;
  }
...
export const removeMembersByGroupId = async ({
  groupId,
}: {
  groupId: number;
}): Promise<void> => {
...
export const findYourGroups = async (userId: number) => {
  return await db
    .select({
      ...
      pictureUrl: groups.pictureUrl,
      members:
        sql`(SELECT COUNT(*) FROM ${groupmembers} WHERE ${groupmembers}.groups_id = ${groups}.id)`.as(
          "members"
        ),
...
export const checkGroupMember = async (groupData: {
  id: number;
  userId: number;
}): Promise<boolean> => {
  const [member] = await db
